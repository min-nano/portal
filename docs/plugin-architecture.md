...
> **実際の分かれ方は上の図と少し違います。** 左の 3 つ（現況検査レポート・
> 構造計算安全証明書・必要壁量）はどれも「雛形を編集して生成物を得る」という
> 同じ流れなので、**1 つのユニット `tool-template-fulfiller` にまとめます**
> （[template-fulfiller.md](template-fulfiller.md)）。この節が言っている
> 「ツールは 3 層をまとめて持つ」という原則は変わりません——変わるのは
> ユニットの数え方だけです。
...
> **第 3・4 段は [template-fulfiller.md](template-fulfiller.md) が差し替えます。**
> 現況検査レポート・構造計算安全証明書・必要壁量の 3 つは、どれも
> 「雛形を編集して生成物を得る」という同じ流れなので、1 つずつ別リポジトリへ
> 出すのではなく **1 つのユニットにまとめ、3 つのツールはそこへ渡す記述
> （レシピ）として portal に置きます**。§1〜§8（3 層の分け方・3 つの継ぎ目・
> 版の固定・循環の避け方・ツール単体でのプレビュー・更新の流れ）は
> そのまま生きています。
