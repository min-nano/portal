...
* **画面（`#app`）を出すのは「入力できるようになった時点」**です。組み立て途中のフォーム――空の選択肢や、まだ並んでいない入力欄――を一瞬見せないために、`#app` は `hidden` のまま始め、フォームを組み立て終えたところで各ページが `showApp()` を呼びます（画面が出て、読み込み中の表示が消えます）。サインインの確認だけで使えるトップページは、その時点で呼びます。
* **失敗して先に進めないときも必ず `showApp()` を呼びます**。理由を出す場所（`#message`）が画面の中にあるためで、回り続けたまま終わる画面を作らないためでもあります。未サインインのとき（サインイン画面を出すとき）は `finishPageLoading()` で消し、代わりにゲートの中へ「サインイン画面を読み込んでいます…」を出します。
...
* **バックエンドを先に起こしておく。** 2 段目は「`/config` → wasm」という直列の並びで、その先頭が **Cloud Run のインスタンス起動待ち**を丸ごと被ります（常時起動のインスタンスは置いていません）。そこで各ツールの `start()` は、サインインの確認を始めるのと同時に、認証の要らない `/api/healthz` を投げます（`api.js` の `warmUpApi()`）。Clerk とやり取りしているあいだに起動が済むので、その分が待ち時間から消えます。失敗しても構いません（起きていなければ、続く `/config` が普通に起こします）。
