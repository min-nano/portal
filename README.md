...
> **計算書 PDF のフォントについて**: 日本語は Adobe-Japan1 の標準 CJK フォントを**埋め込まずに**参照しています（フォントファイルを同梱せずに済ませるため）。文字コードは既定 CMap から一意に定まるので検索・コピー・テキスト抽出は正しく行えますが、日本語フォントの無い環境では字形が代替されます。
