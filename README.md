...
**面材と釘の仕様は面材 1 枚ごと**に決めます。1 枚の壁でも、上半分は N50・下半分は CN50 のように**張り分けることがある**ためです。壁が持つのは**面材を張る軸組**（階高 H・壁の幅 W・間柱/根太ピッチと、**軸組材の見付け幅**）だけで、面材の厚さ `t`・`GB`・釘の `k`・`δv`・`δu`・`ΔPv`・`τmax`・`E1`・`E2` は面材ごとの入力欄にあります（面材ごとの節の中に「面材と釘」と「壁の中で占める領域と釘」が入ります）。
...
* **軸材の縁端距離**（釘の中心から軸組材の縁まで）は、**壁の軸組材の見付け幅**から判定します。3.3(1)④ の残り半分「軸材の釘列に対する縁端距離は、**20mm 以上かつ接合具径 d [mm] × 5 以上**」です（下の「軸組材の見付け幅から、軸材の縁端距離が決まる」を参照）
...
**軸組材の見付け幅から、軸材の縁端距離が決まる**

* 壁の入力に、面材を張る**軸組材の見付け幅**（面材の側から見た材の幅）が入ります。**柱**（壁の両端の縦材）・**間柱**（中間の縦材）・**横架材**（壁の上下の横材）・**継目の材**（壁の内側で面材どうしが突き付く縁を受ける材）の 4 つで、既定は尺モジュールの在来軸組でよくある取り合わせ（柱 105・間柱 45・横架材 105・継目の材 105 mm）です
* 釘を打つ線は面材の四周とその面材にかかる間柱で、**どの線も軸組材の心の上**にあります（面材どうしは軸組材の心で突き付けて張るため）。面材の縁の釘列だけがへりあきぶん内側へずれるので、縁端距離はそのまま引き算で出ます

  ```text
  縁端距離 = 見付け幅 ÷ 2 − へりあき   … 面材の縁の釘列（左右の縁・上下の縁）
  縁端距離 = 見付け幅 ÷ 2              … 中間の間柱の釘列（材心の上に打つ）
  ```

* どの材に来るかは、その釘列が壁のどこにあるかで決まります。縦線は**壁の両端なら柱・壁の内側なら継目の材**、中間の縦列は**間柱**、横線は**壁の上下端なら横架材・壁の内側なら継目の材**です。継目の材を間柱と別の入力にしてあるのは、継目が 2 枚ぶんの縁の釘列を 1 本の材で受けるぶん広い見付けが要るため（適用範囲 3.3(1)⑥ も「端部及び継目の材」としてまとめて断面を定めています）
* 判定は面材 1 枚ずつ釘列ごとに行い、**いちばん余裕の少ない釘列**を壁の判定にします。計算書と画面には、その面材・釘列・材の見付け幅まで出します（`最小 縁端距離 15 mm < 20 mm（面材「上段」の中間の間柱 ／ 間柱 見付け 30 mm ／ 20 mm かつ 釘の呼び径 φ3.05 mm × 5 以上）`）。面材のページにも、その面材でいちばん厳しい釘列が 1 行残ります
* NG のとき、**広げるべきなのが面材のへりあきなのか材の見付けなのか**が、この 1 行で分かります（中間の間柱の釘列は材心に打つので、へりあきをいくら動かしても変わりません）
...
* 軸組材は**釘配列そのものには効きません**。剛性・許容せん断耐力の数値は軸組材を入れる前と変わらず、増えるのは判定だけです（軸組材を持たない版で保存した計算書を開いたときは、既定の見付け幅で判定が始まります）
...
core/                         # ポータルの計算の唯一の実装（Rust → wasm）
  ...
  src/frame.rs                # 壁の軸組材（柱・間柱・横架材・継目の材）と釘列の縁端距離
