...
* 面材 1 枚ごとに、**張る面（表面 / 裏面）と、壁の左下を原点とした面材の左下の位置 X・Y** を入れられます。入れると、画面と計算書に **壁の面材配列図**（壁の枠の中に面材を並べた姿図）と**面材の一覧**（面材・張る面・寸法・左下の位置・面積）が付きます
* 計算（3.3）は面材ごとの値の**和**なので、**配置そのものは数値を変えません**（枚数だけでも計算できます）。それでも入力できるようにしてあるのは、**どういう張り方を前提にした計算なのかが計算書から伝わる**ようにするためと、**配置と計算の食い違い**をその場で拾うため
* 判定（**面材の配置**）で次を検定します。どれも「計算に入れた面材と、想定している張り方が合っていない」印です
  * 面材が**壁からはみ出している**（壁の寸法か面材の寸法のどちらかが違う）
  * 同じ面に張った面材どうしが**重なっている**（枚数を二重に数えている）
  * 配置を入れた面材と入れていない面材が**混ざっている**（図が計算より少ない）
...
* **配置は任意入力**です。1 枚も入れていない壁は今までどおり枚数だけで計算し、配列図のページも判定の行も出しません。面材を追加したときは、直前の面材に位置が入っていればその**真上**を初期値にします（壁は下から段を重ねて張ることが多いため）
...
* **1 ファイル = 1 物件**。物件の中の複数の壁をページ送りで切り替えて編集し、**計算書 PDF では壁ごとに、面材の配置を入れていれば「1 ページ = 壁の面材配列図」を先頭に置き、続けて「1 ページ = 1 面材（釘配列諸定数）」を並べ、最後に「1 ページ = その壁」**を置く（配列図がその壁の張り方を示し、壁の計算の根拠になる釘配列諸定数が、必ずその直前のページにそろう）。面材のページには、その面材に使った**面材と釘**と**壁内の位置**もそのまま載る
...
# 壁内の位置（面材の左下）を入れると、先頭に壁の面材配列図が 1 ページ増える。
panels = data["walls"][0]["panels"]
panels[0]["originX"], panels[0]["originY"] = 0, 0        # 下段
panels[1]["originX"], panels[1]["originY"] = 0, 1820     # その上に重ねる
data = panel_shear.normalize_data(data)
open("配列図つき.pdf", "wb").write(panel_shear.build_pdf(data, panel_shear.validate(data)))
