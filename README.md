...
**面材と釘の仕様は面材 1 枚ごと**に決めます。1 枚の壁でも、上半分は N50・下半分は CN50 のように**張り分けることがある**ためです。壁が持つのは**面材を張る軸組**（階高 H・壁の幅 W と、**軸組材そのもの**）だけで、面材の厚さ `t`・`GB`・釘の `k`・`δv`・`δu`・`ΔPv`・`τmax`・`E1`・`E2` は面材ごとの入力欄にあります（面材ごとの節の中に「面材と釘」と「壁の中で占める領域と釘」が入ります）。
...
* **面材のせん断破壊・せん断座屈の検定**（式 3.3.8〜3.3.11）を面材 1 枚ごとに行う。`τN = Cxy・Zxy・ΔPv / t` が、面材のせん断強度 `τmax` と臨界せん断座屈応力度 `τcr` の両方を下回ることを確かめ、a・b・β・τN・τmax・τcr を表にして残す
  ...
  * ξ は中間材（間柱等）の有無で 2 / 1。**「中間材を設けるか」は入力しません**（壁の幅の内側に縦材が 1 本でも立つかどうかで決まります）。入れた軸組材と食い違ったまま計算できてしまう欄を無くすためです。a（繊維直交方向の長さ）と b（繊維平行方向の長さ）は、面材ごとに選ぶ繊維方向（既定は長辺方向）から決まる
* 適用範囲（3.3(1)）のうち機械的に判定できるものを検定する。**①許容せん断耐力の上限 13.72 kN/m** と、**④の面材の釘列に対するへりあき**・**④の軸材の釘列に対する縁端距離**（どちらも下記）の 3 つ。残り（面材と釘の組合せ・釘のピッチ・端部および継目の材の断面・中間材の配置）は設計者が確認する前提で、計算書の脚注に明記する

**壁内の面材配列（壁のどこに何を張るか）**

* 面材 1 枚ごとに、**張る面（表面 / 裏面）**と**壁の中で占める領域**（左下 (X, Y) と右上 (X, Y)）を入れます。画面と計算書には **壁の面材配列図**（壁の枠の中に面材を並べ、軸組材を破線で重ねた姿図）と**面材の一覧**（面材・張る面・寸法・左下の位置・面積）が付きます
...
**釘配列は壁の軸組から決まる（型の入力欄はない）**

* 釘を打つ線は、**面材の四周**（面材張り大壁は適用範囲 3.3(1)⑤ で四周打ち）と、**その面材の内側を通る縦材**です。軸組材は 1 本ずつ位置を入れるので（下記）、**軸組材と面材の占有領域が決まれば釘座標が決まります**
* そのため、**配列の型（川型・山型・ロ型・日型）を選ぶ欄も、格子・座標を直接入力する欄もありません**。面材ごとに残る釘の入力は**釘ピッチとへりあき**だけです。型は結果として決まるもので、設計者が選ぶものではないという整理です
* 同じ面材でも、壁の中で置く場所が変われば釘の縦列が変わります（縦材をまたぐ位置なら中間の縦列が入り、縦材と縦材のあいだに収まるなら入りません）
* 面材の長辺方向に走る間柱の釘列は、3.3(1)⑧ により釘配列計算に含めません（縦長の面材では中間の縦列を外します）
* 計算書には、その面材の釘配列が何から決まったのか（`四周打ち ＋ 中間の縦材（この面材にかかる縦列 2 本） ／ 釘 @75 ／ へりあき 10 mm`）をそのまま残します
...
* **軸材の縁端距離**（釘の中心から軸組材の縁まで）は、**壁に入れた軸組材**から判定します。3.3(1)④ の残り半分「軸材の釘列に対する縁端距離は、**20mm 以上かつ接合具径 d [mm] × 5 以上**」です（下の「軸組材から、軸材の縁端距離が決まる」を参照）
* 途中経過（x0, y0, Ix, Iy, Zx, Zy, αx, Zpxy …）も式番号つきですべて表示し、計算のブラックボックス化を防ぐ（白箱化）
* **グレー本 表 3.2.1「標準的なサイズの面材の釘配列諸定数」の組み合わせを、一覧から呼び出せる**（面材寸法・間柱/根太ピッチ・釘ピッチの 33 通り）。呼び出すと**壁の軸組材がそのピッチの等間隔で入り直し**、**その面材の釘ピッチ・へりあき・大きさ**（左下はそのままに右上を動かす）が入るので、そこから実際の設計に合わせて動かせる。配列の型は面材が壁のどこに来るかで決まるので、一覧には型が出てこない（表そのものは 106 通りで、型を含む全件との突き合わせは計算実装のテストにある）

**軸組材は 1 本ずつ、自由な位置に入れる**

* 壁の入力は、面材を張る**軸組材の一覧**です。1 本ごとに **種別**（横架材・柱・継目の材・間柱）・**向き**（縦材／横材）・**名前**（計算書にそのまま出ます）・**材心の位置**（壁の左下が原点。縦材は X、横材は Y）・**見付け幅**（面材の側から見た材の幅）・**材端**（材の長さの方向の位置。縦材は Y、横材は X）を入れます。等間隔とは限らない実際の軸組（開口の脇に寄せた縦材、面材の継目に入れる受け材、まぐさ・窓台）をそのまま書けます
* **材端の既定は、直交する材のいちばん外の面まで**です。在来軸組の納まりのとおり、**横架材は両端の柱の外面まで伸び**、柱・間柱は上下の横架材の外面まで伸びます（直交する材が壁の中にしか無ければ、壁のその辺まで）。まぐさや窓台のように途中で終わる材は材端を直せばよく、欄を空にすれば既定に戻ります。材端は図に描く長さだけを決め、計算（釘の縦列・縁端距離）には効きません
* **種別は計算に効きません**（釘の縦列も縁端距離も、向き・位置・見付け幅で決まります）。効くのは**図の勝ち負け**（下記）と、軸組材を足すときの既定（名前・向き・見付け幅）です。名前は自由なので、「通し柱」「まぐさ」のように書いても、種別で描き方が決まります
* 尺モジュールのように等間隔で並べたいときは、**間柱ピッチを入れて「等間隔で入れ直す」**と、壁の両端に柱・その間に間柱・上下に横架材が一度に入ります（入れたあとは 1 本ずつ動かせます）。表 3.2.1 の組み合わせを呼び出したときも、その配列が前提とするピッチで入り直します
* **釘の縦列**は、面材の左右の縁と**その面材の内側を通る縦材**に入ります。せん断座屈の ξ も、壁の中に縦材が 1 本でも立てば 2 になります
* **図にも軸組材が出ます**。壁の面材配列図（壁の姿図）にも、面材ごとの釘配列図にも、その位置と見付け幅のまま**破線**で重ねて描きます（面材の裏に隠れているものなので塗りません）。釘配列図では釘と重ねて描くので、**へりあき**（面材の縁から釘まで）と**軸材の縁端距離**（釘から材の縁まで）が図の上で見えます
* **交わるところは、種別の勝ち負けで描き分けます**。在来軸組の納まりのとおり

  ```text
    横架材 ＞ 柱 ＞ 継目の材 ＞ 間柱
  ```

  で、**勝ったほうが通り、負けたほうはその手前で止まります**（管柱は横架材で切られ、面材の継目の受け材は柱の間に納まり、間柱はその受け材で切られる）。負けた材は 2 片以上に分かれて描かれます。同じ種別どうし（縦の継目の材と横の継目の材など）は、どちらも通して描きます
* 図を描く範囲には、軸組材が壁や面材の外へ出るぶんまで入れます（両端の柱・上下の横架材は材心が壁の縁に立つので、見付け幅の半分が外へ出ます）。図の縁で材が切れたり、寸法の文字と重なったりしません。面材 1 枚の釘配列図では、その面材のまわり（面材にかかる直交材の出っぱりまで）で切って描きます

**軸組材から、軸材の縁端距離が決まる**

* 適用範囲 3.3(1)④ の残り半分「軸材の釘列に対する縁端距離は、**20mm 以上かつ接合具径 d [mm] × 5 以上**」を判定します。これまでは軸材の断面が要るため設計者が確認する前提にしていたものです
* 縁端距離は、**その釘列を受ける軸組材の縁まで**の距離として測ります。中間の縦材の釘列は材心の上に来るので見付け幅の半分、面材の縁の釘列は面材の縁からへりあきぶん内側に来るので、材心とのずれのぶんだけ縮みます
...
* 釘は面材の四周に打つので（3.3(1)⑤）、**面材の縁が来るところには必ず軸組材が要ります**。受ける材が無い釘列は「**軸組材なし**」として、いちばん厳しい釘列と同じ扱いで NG にします（面材の継目に受け材を入れ忘れている、という印です）
* 判定は面材 1 枚ずつ釘列ごとに行い、**いちばん余裕の少ない釘列**を壁の判定にします。計算書と画面には、その面材・釘列・材の名前と見付け幅まで出します（`最小 縁端距離 15 mm < 20 mm（面材「上段」の中間の縦材（X = 455 mm） ／ 間柱（見付け 30 mm） ／ 20 mm かつ 釘の呼び径 φ3.05 mm × 5 以上）`）。面材のページにも、その面材でいちばん厳しい釘列が 1 行残ります
* NG のとき、**広げるべきなのが面材のへりあきなのか材の見付けなのか**が、この 1 行で分かります（中間の縦材の釘列は材心に打つので、へりあきをいくら動かしても変わりません）
* グレー本 3.3(3) の計算例（間柱 30 × 105、図 3.3.10）を再現した計算書では、**この行は NG と出ます**。中間の間柱に打つ釘は材心に来るため縁端距離が 15 mm しか取れず、20 mm に届きません（間柱を 45 にすれば 22.5 mm で通ります）
* 軸組材は**釘配列そのものの規則を変えません**（釘の縦列がどこに入るかは前と同じ「面材の四周＋中間の縦材」です）。**軸組材を持たない版で保存した計算書**は、開いたときに当時の前提のまま読み替えます（壁の両端に柱・間柱ピッチで間柱・上下に横架材、それに面材の継目の材）。**材端を持たない版**で保存した計算書は、開いたときに既定の材端（直交する材の外面まで）が入ります。実際の納まりに合わせて、開いたあとに 1 本ずつ直せます

**共通**

* **1 ファイル = 1 物件**。物件の中の複数の壁をページ送りで切り替えて編集し、**計算書 PDF では壁ごとに「1 ページ = 壁の面材配列図」を先頭に置き、続けて「1 ページ = 1 面材（釘配列諸定数）」を並べ、最後に「その壁のページ」**を置く（配列図がその壁の張り方を示し、壁の計算の根拠になる釘配列諸定数が、必ずその直前のページにそろう）。面材のページには、その面材に使った**面材と釘**と**壁内の配置**もそのまま載る
  * 壁のページに載る量は**面材の枚数で変わる**（面材ごとの表が 3 つある）ので、1 ページに収まらなければ**「（続き）」のページへ送ります**。節や表の途中では切らず、続きのページにも見出し・物件名・壁の名前を繰り返すので、そのページだけを見てもどの壁の続きかが分かります。判定は読み手がいちばん先に探すところなので、**節ごと**次のページへ送ります
* 計算書には入力・釘配列諸定数・途中経過に加えて、**釘配列図**（面材の枠・釘・弾性中立軸・軸組材）と、**壁の面材配列図**（壁の枠・面材・張る面・軸組材）を描く
* **前の版で保存した PDF も開けます**。釘配列パターンを別に登録して壁から選んでいた形の入力は、読み込み時に「壁が面材そのものを持つ」今の形へ移し替えます（どの壁からも使われていなかったパターンは、面材 1 枚だけの壁として残るので、面材と釘の数値を入れ直してください）。面材と釘を**壁が 1 組だけ持っていた**形の入力も、読み込み時にその壁の全ての面材へ配ります（当時は壁の中で仕様が混在しえなかったので、計算はそのまま一致します）
  * **面材ごとに寸法を持っていた形**の入力は、「壁の中で占める領域」へ移し替えます。壁の中の位置を持っていなければ、**張る面ごとに下から順に積みます**（重ならないように。壁より高く積み上がれば、そのぶんは「はみ出し」として判定に出ます）。面材ごとにあった**間柱ピッチ**は、壁の**軸組材**へ読み替えます（下記）
...
core/                         # ポータルの計算の唯一の実装（Rust → wasm）
  ...
  src/layout.rs               # 釘の割り付け（型・縦材の位置・ピッチ・へりあき → 釘座標）
  src/frame.rs                # 壁の軸組材（1 本ずつ自由な位置）と、釘列の縁端距離
