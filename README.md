...
設計等業務の見積書（御見積書）を作り、Drive に PDF で保存します。**耐震診断・耐震補強設計にも対応します。**

建築士法第 24 条の 10（**努力義務**）が求める「報酬の算定の根拠を示す見積書」にあたる書面です。ここで作るのは見積書そのもので、業務報酬内訳書ではありません（そちらは [docs/contract-formatter.md](docs/contract-formatter.md) の見積書・契約書等 作成ツールが担います）。

* **業務のテンプレート**を選ぶと、規模・床面積・設計方法（または診断法）から**品名と摘要の下書き**が付いてきます。扱うのは、構造設計（構造計算＋構造図）・構造計算のみ・壁量計算＋基礎構造図・基礎の構造設計・**耐震診断**・**耐震補強設計**・変更設計料・その他（自由記述）の 8 つ
* 下書きは**候補**であって、書き換えた欄は上書きされません（規模や設計方法を動かすと、手を入れていない欄だけが付いてきます）。「候補に戻す」でいつでも戻せます
* **金額・消費税・合計は入力のたびに計算します**。消費税は**税率の区分ごとに 1 度だけ**丸めるので、内訳の税額と合計の税額が食い違いません。立替金などの**課税対象外**の行も並べられます
* **消費税の条件（税率・軽減税率・端数処理）は見積書そのものが持ちます**。設定は新規作成時の初期値で、あとから設定を変えても作成済みの見積書は変わりません
* **既定のファイル名は `YYYYMMDD_取引先名_金額.pdf`**。そのまま保存すれば、電子帳簿保存法の検索要件（取引年月日・取引先・取引金額）への備えになります
* 有効期限は「発行日の翌月末」を 1 押しで入れられます
* ファイル操作は他の PDF ツールと同じ（新規作成 / 開く / 保存 / 別名で保存）。**フォーム入力は PDF の文書情報へ埋め込まれる**ので、保存した見積書を開き直せば続きを編集できます（枝番の見積書も、元の PDF を開いて番号と金額を直すだけで作れます）
* 雛形は使いません。明細の数だけ行が伸びる書面なので、バックエンドが直接組み立てます（釘配列諸定数の計算書と同じ作法。`backend/app/pdf_write.py`）。明細が 1 頁に収まらなければ頁を送り、**合計欄が途中で切れることはありません**

#### 耐震診断・耐震補強設計の報酬（告示第670号）

耐震診断・耐震補強設計の明細では、**平成 27 年国土交通省告示第 670 号**（建築物の耐震診断及び耐震改修に係る設計等に関する標準業務及び報酬）の略算方法から**報酬の参考額**を出せます。

```
標準業務人・時間数   別添二 別表第二（戸建木造住宅、床面積の合計 75〜250㎡）
                     耐震診断 45 人・時間 ／ 耐震改修に係る設計 60 人・時間
直接人件費         = 人・時間数 × 一人・一時間当たりの人件費
直接経費 + 間接経費 = 直接人件費 × 1.0（第四 ロ。告示第8号の 1.1 とは違う）
技術料等経費       = 直接人件費 × 技術料等経費率
検査費・特別経費    = 実費（入力）
業務報酬（税抜）    = 上の合計
```

* **この表は範囲に対する 1 つの値なので、補間しません。** 床面積が 75〜250㎡ の外なら、**参考値も出しません**（外挿の根拠が無いため）。その旨を画面に出して、実費を積み上げてもらいます
* **別表第一（鉄骨造・鉄筋コンクリート造・鉄骨鉄筋コンクリート造）は実装していません。** 先行実装は刻みの間を `A = a × S^b` で埋めていますが、その係数の出どころを原文で確かめられていないためです（[docs/contract-formatter.md](docs/contract-formatter.md) §6.7）
* **出るのは参考額**で、「単価に入れる」を押して初めて明細に入ります。入ったあとは手で直せます
* **告示に定めがあるのは標準業務人・時間数と費目の構成、そして倍数 1.0 まで**です。人件費単価と技術料等経費率に定めはなく、事務所が決める値として共有設定に置きます（未設定なら参考額を出しません。0 円の参考額を出すより、設定を促すほうが安全です）

#### 何がリポジトリにあり、何が設定にあるか

**このリポジトリは公開です。** 見積書という書類の性質から、線を引いてあります（[docs/contract-formatter.md](docs/contract-formatter.md) §8）。

| | リポジトリ（公開） | 共有設定（Firestore・非公開） |
| --- | --- | --- |
| 報酬算定 | 告示第670号の別表・費目・倍数（`core/src/kokuji670.rs`） | 人件費単価・技術料等経費率 |
| 書式 | 見積書の体裁、業務の標準的な語彙（設計ルート・診断法） | 事務所の**定型文**（摘要の但し書き・備考） |
| 事務所 | — | 名称・所在地・電話番号・担当者名 |
| 税 | 法定の税率（10% / 8%）の既定 | 端数処理の決め |

**リポジトリ側の既定値は空です。** 事務所の名称も定型文も単価も入っていないので、**使い始めるにはまず画面の「設定」から登録します**。テストも架空の値だけで回っています。

**印字されるのは、見積書が持っている値だけです。** 設定は新規作成時の初期値として写されるだけで、生成のときに読み直したりはしません。事務所名を変えても、過去の見積書の再現性は壊れません。
...
frontend/                     # Firebase Hosting に載せる SPA (Vite)
  ...
  src/quotation-formatter/    # 見積書の入力・明細の組み立て・告示第670号の参考額
...
backend/                      # Cloud Run サービス (FastAPI)
  ...
  app/quotation.py            # 見積書 PDF の組み立てと解析・共有設定の整え
...
core/                         # ポータルの計算の唯一の実装（Rust → wasm）
  ...
  src/quotation.rs            # 見積書の金額・消費税と、業務ごとの摘要の組み立て
  src/kokuji670.rs            # 耐震診断・耐震改修の標準業務人・時間数（平成27年国交省告示第670号）
...
# 保存時の突き合わせ・雛形設定・JWT 検証、見積書 PDF の往復と頁送り・共有設定、
# 必要壁量ツール（同梱した配布物へ実際に
