...
* 壁の入力は、面材を張る**軸組材の一覧**です。1 本ごとに **種別**（横架材・柱・継目の材・間柱）・**向き**（縦材／横材）・**名前**（計算書にそのまま出ます）・**材心の位置**（壁の左下が原点。縦材は X、横材は Y）・**見付け幅**（面材の側から見た材の幅）を入れます。等間隔とは限らない実際の軸組（開口の脇に寄せた縦材、面材の継目に入れる受け材、まぐさ・窓台）をそのまま書けます
...
* 図を描く範囲には、軸組材が壁や面材の外へ出るぶんまで入れます（両端の柱・上下の横架材は材心が壁の縁に立つので、見付け幅の半分が外へ出ます）。図の縁で材が切れたり、寸法の文字と重なったりしません
...
* 軸組材は**釘配列そのものの規則を変えません**（釘の縦列がどこに入るかは前と同じ「面材の四周＋中間の縦材」です）。**軸組材を持たない版で保存した計算書**は、開いたときに当時の前提のまま読み替えます（壁の両端に柱・間柱ピッチで間柱・上下に横架材、それに面材の継目の材）。実際の納まりに合わせて、開いたあとに 1 本ずつ直せます
