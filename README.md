...
グレー本『木造軸組工法住宅の許容応力度設計』の **3.2 面材張り耐力要素の詳細計算法で用いる釘配列諸定数の計算**（式 3.2.1〜3.2.7）と、**3.3 面材張り大壁の詳細計算法**（式 3.3.1〜3.3.11）に沿って、釘配列諸定数 **Ixy・Zxy・Cxy** と、それを使った壁の面内せん断剛性 **K**・許容せん断耐力 **Pa** を求めます。GAS 版 [gas-timber-panel-shear-calculator](https://github.com/min-nano/gas-timber-panel-shear-calculator) の移植（3.2）と、その先の拡張（3.3）です。
...
* 面材と釘の組合せは **グレー本 表 3.3.1「面材釘 1 本あたりの一面せん断の数値」の 12 通りを一覧から読み込める**（表 3.3.2 の既定の規格も一緒に入るので、1 回の選択で検定まで数値がそろう）。表にない組合せは、グレー本 4.5 の試験で求めた `k`・`ΔPv`・`δv`・`δu` を直接入力できる
* **面材のせん断破壊・せん断座屈の検定**（式 3.3.8〜3.3.11）を面材 1 枚ごとに行う。`τN = Cxy・Zxy・ΔPv / t` が、面材のせん断強度 `τmax` と臨界せん断座屈応力度 `τcr` の両方を下回ることを確かめ、a・b・β・τN・τmax・τcr を表にして残す
  ...
  * `τmax`・`E1`・`E2` は **グレー本 表 3.3.2「面材のせん断強度及び曲げヤング係数」から読み込める**（構造用合板は JAS 1 級 / 2 級を選べる）
  * ξ は中間材（間柱等）の有無で 2 / 1。a（繊維直交方向の長さ）と b（繊維平行方向の長さ）は、面材ごとに選ぶ繊維方向（既定は長辺方向）から決まる
* 適用範囲（3.3(1)）のうち機械的に判定できる**①許容せん断耐力の上限 13.72 kN/m** も検定する。②〜⑧（面材と釘の組合せ・釘のピッチとへりあき・端部および継目の材の断面・中間材の配置）は設計者が確認する前提で、計算書の脚注に明記する
