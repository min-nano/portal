...
class Core {
  ...
  /**
   * 軸組材（柱・間柱・横架材）を、間柱ピッチで等間隔に組み立てる。
   *
   * 軸組材は 1 本ずつ自由な位置に入れられるので、これは「よくある並びを
   * 一度に入れる」ための入り口（新しい壁の初期値・表 3.2.1 の呼び出し・
   * 画面の「等間隔で入れ直す」）。
   */
  frame({ width, height, studPitch }) {
    return this.call({ op: 'frame', data: { width, height, studPitch } }).frame;
  }
