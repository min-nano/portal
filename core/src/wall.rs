...
//! 機械的に確かめられるのでここで判定する。④のへりあき・縁端距離は、面材の
//! 釘座標（layout）と壁の軸組材の見付け幅（frame）から判定する。残り（面材と
//! 釘の組合せ、釘のピッチ、端部および継目の材の断面、中間材の配置など）は
//! 寸法と納まりの話なので、設計者が確認する前提とする。
...
/// **20mm 以上かつ接合具径 d [mm] × 5 以上**とする」。
///
/// 面材側（面材の縁から釘の中心まで）は面材の寸法と釘座標だけで測れる。
/// 軸材側は釘が刺さる軸組材の見付け幅が要るので、壁の入力として軸組材の
/// 寸法を受け取り、frame module が釘列ごとの縁端距離を出す。
pub const MIN_EDGE_DISTANCE: f64 = 10.0;
/// 同じく 3.3(1)④ の、軸材の釘列に対する縁端距離の最小値 [mm]。
pub const MIN_FRAME_EDGE_DISTANCE: f64 = 20.0;
/// 同じく 3.3(1)④ の、接合具径に対するへりあき・縁端距離の倍率。
